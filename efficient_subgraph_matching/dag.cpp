...
int *idxSortedData = NULL; //idxSortedData[l] contains the start index in sortedData such that labelData[sortedData[index]] is l
...
int cand(int i) {
    int label = labelQuery[i];
    int degree = degreeQuery[i];
...
double rRank(int i) {
    ...
    return numInitCand + 1/(double)degree;
}


double rRank_original(int i) {
    int label = labelQuery[i];
    int degree = degreeQuery[i];
    ...
    int numInitCand = end - mid;
    return numInitCand/(double)degree;
}

bool sortByCand(int aNode1, int aNode2) {
    return cand(aNode1) < cand(aNode2);   // less candidate is better
}

bool sortByrRank(int aNode1, int aNode2 ) {
    return  rRank(aNode1) < rRank(aNode2);      // lower rank is better
}

bool sortByrRank_original(int aNode1, int aNode2 ) {
    return  rRank_original(aNode1) < rRank_original(aNode2);      // lower rank is better
}
...
bool sortByDegreeQuery(int aNode1, int aNode2) {
    return (degreeQuery[aNode1] > degreeQuery[aNode2]);  // bigger degree is better
}
...
bool sortByLabelFreqQuery(int aNode1, int aNode2) {
    ...
    return (labelFrequency[label1] < labelFrequency[label2]);   // less frequency is better
}
...
//construct dag data structure
    ...
    while (true) {
        ...
        stable_sort(queue + currQueueStart, queue + currQueueEnd, sortByrRank);
        ...
        while (currQueueStart != currQueueEnd) {
            ...
            for (int i = adjIndexQuery[currNode]; i < adjIndexQuery[currNode + 1]; ++i) {   // maybe the adj array in the class
...
int selectRoot() {
    ...
    for (int i = 0; i < numQueryNode; ++i) {
        ...
        int numChild = adjIndexQuery[i + 1] - adjIndexQuery[i];

        for (int j = adjIndexQuery[i]; j < adjIndexQuery[i + 1]; ++j) {
            int childNode = adjListQuery[j];
            averageRank += rRank(childNode) / numChild;

            int numGrandChild = adjIndexQuery[childNode + 1] - adjIndexQuery[childNode];
            for (int k = adjIndexQuery[childNode]; k < adjIndexQuery[childNode + 1] ; ++k) {
                int grandSonNode = adjListQuery[k];

                averageRank += rRank(grandSonNode) / (numChild * numGrandChild) ;
        ...
        rank =  numInitCand + averageRank * 1/3 ;



        // get root that minimize rank ,   less candidate, more degree is better
        if (rank < rootRank) {
            root = i;
            rootRank = rank;
        }
